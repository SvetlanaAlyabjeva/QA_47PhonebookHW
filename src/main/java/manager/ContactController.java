...
public class ContactController implements BaseAPI {
    ...
    public void login() {
        ...
        Response response = new AuthenticationController().requestRegLogin(user, LOGIN_URL);
         System.out.println("ContactController login" + response.getStatusLine());
    ...
    public Response getAllUserContacts(TokenDto tokenDto) {
        return given()
                .baseUri(getProperty("login.properties", "baseUri"))
                .accept(ContentType.JSON)
                .header("Authorization", tokenDto.getToken())
    ...
    protected Response deleteAllContacts(TokenDto tokenDto) {
        return given()
                .log().all()
                .baseUri(getProperty("login.properties", "baseUri"))
                .accept(ContentType.JSON)
                .header("Authorization", tokenDto.getToken())
                .delete(ADD_NEW_CONTACT_URL + "/clear")
                .thenReturn()
                ;
    }
